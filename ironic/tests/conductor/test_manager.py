...
class ManagerTestCase(tests_db_base.DbTestCase):
    ...
    def test_deploy_validate_power_state(self):
        ...
        with task_manager.acquire(self.context,
                                  [node.uuid]) as task:
            self.driver.deploy.validate_power_state(task, node, 'deploy')
    ...
    def test_destroy_validate_power_state(self):
        ...
        with task_manager.acquire(self.context,
            ...
            self.driver.deploy.validate_power_state(task, node, 'destroy')

    def test_destroy_validate_power_state_update_invalid(self):
        ndict = utils.get_test_node(driver='fake',
                                    power_state=states.POWER_ON)
        node = self.dbapi.create_node(ndict)
        with task_manager.acquire(self.context,
