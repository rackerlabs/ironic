...
class TestAgentDeploy(db_base.DbTestCase):
    ...
    @mock.patch('ironic.common.tftp.update_neutron')
    ...
    @mock.patch('ironic.drivers.modules.agent._get_client')
...
class TestAgentVendor(db_base.DbTestCase):
    ...
    @mock.patch('ironic.drivers.modules.agent._get_client')
    def test_continue_deploy(self, get_client_mock, image_service_mock):
        instance_info = db_utils.get_test_agent_instance_info()
        ...
        self.passthru._continue_deploy(self.task)
        client_mock.prepare_image.assert_called_with(self.node,
            expected_image_info,
            instance_info['configdrive'])
        self.assertEqual(self.node.provision_state, states.DEPLOYING)
    ...
    def test_heartbeat(self):
        ...
        self.passthru._heartbeat(self.task, **kwargs)
