...
INFO_DICT = db_utils.get_test_agent_info()
...
class TestAgentDeploy(db_base.DbTestCase):
    ...
    def test_validate_fail(self):
        del self.node.instance_info['agent_url']
    ...
    def test_deploy(self, get_client_mock, image_service_mock, power_mock,
                    bootdev_mock):
        instance_info = db_utils.get_test_agent_info()
        ...
        client_mock.prepare_image.assert_called_with(self.node,
            expected_image_info,
            instance_info['configdrive'],
            wait=True)
        power_mock.assert_called_with(self.task, self.node, states.REBOOT)
        bootdev_mock.assert_called_with(self.task, 'disk')
    ...
    def test_tear_down(self, power_mock, bootdev_mock):
        ...
        bootdev_mock.assert_called_with(self.task, 'pxe')
    ...
    def test_prepare(self, get_client_mock):
        with task_manager.acquire(self.context, [self.node['uuid']],
            ...
            driver_return = self.driver.prepare(task, self.node)
...
class TestAgentVendor(db_base.DbTestCase):
    ...
    def test_lookup_unversioned_success(self, mocked_lookup):
        ...
        mocked_lookup.assert_called_once_with(self.passthru, task, **kwargs)
    ...
    def test_lookup_v0(self, find_mock):
        ...
        node = self.passthru._lookup(FakeTask(), **kwargs)
    ...
    def test_heartbeat(self):
        ...
        self.passthru._heartbeat(self.task, self.node, **kwargs)
    ...
    def test_flatten_dict(self):
        ...
        flattened_hardware = self.passthru._flatten_dict(hardware)
    ...
    def test_save_hardware(self, flatten_mock):
        ...
        self.passthru._save_hardware(self.context, self.node, hardware,
                                     version=2)
        self.assertEqual(self.node.extra, expected_hardware)

    @mock.patch('ironic.drivers.modules.agent.AgentVendorInterface.'
                '_flatten_dict')
    def test_save_ports(self, flatten_mock):
        interfaces = [
            {
                'mac_address': 'aa:bb:cc:dd:ee:fe',
                'name': 'eth0',
        ...
        expected_interfaces = {
            '0/mac_address': 'aa:bb:cc:dd:ee:fe',
            '0/name': 'eth0',
            '0/switch_chassis_descr': 'tor1',
            '0/switch_chassis_id': '1',
            '0/switch_port_descr': 'port24',
            '0/switch_port_id': '24',
            '1/mac_address': 'aa:bb:cc:dd:ee:ff',
            '1/name': 'eth1',
            '1/switch_chassis_descr': 'tor2',
            '1/switch_chassis_id': '2',
            '1/switch_port_descr': 'port24',
            '1/switch_port_id': '24',
        }
        flatten_mock.return_value = expected_interfaces

        ports = self.passthru._save_ports(self.node, interfaces)
