...
class TestAgentNeutronAPI(db_base.DbTestCase):
    ...
    @mock.patch('ironic.drivers.modules.agent_utils.AgentNeutronAPI.'
                '_get_node_portmap')
    def test_add_provisioning_network(self, portmap_mock, create_port_mock):
        portmap_mock.return_value = self.fake_portmap
        expected_params = {
            'switch:portmaps': self.fake_portmap,
    ...
    @mock.patch('ironic.drivers.modules.agent_utils.AgentNeutronAPI.'
                '_get_node_portmap')
    def test_add_provisioning_network_no_portmap(self, portmap_mock):
    ...
    def test_add_provisioning_network_connection_failed(
            self, portmap_mock, create_port_mock):
        portmap_mock.return_value = self.fake_portmap
    ...
    def test_remove_provisioning_network(self, list_ports_mock,
        ...
        delete_calls = [
            mock.call(fake_list_ports['networks'][0]['id']),
            mock.call(fake_list_ports['networks'][1]['id'])
        ]
    ...
    @mock.patch('neutronclient.v2_0.client.Client.list_ports')
    def test_remove_provisioning_network_no_ports(self, list_ports_mock):
        list_ports_mock.return_value = {'networks': []}
        self.assertRaises(exception.NetworkError,
                          self.api.remove_provisioning_network,
                          self.node)
    ...
    @mock.patch('neutronclient.v2_0.client.Client.update_port')
    def test_deconfigure_instance_networks(self, update_port_mock):
        self.api.dbapi = mock.Mock()
        self.api.dbapi.get_ports_by_node_id.return_value = self.fake_ports
        expected_params = {
            'switch:portmaps': [],
            'switch:hardware_id': None,
            'switch:commit': False
        }
    ...
    def test_deconfigure_instance_networks_no_ports(self):
        self.api.dbapi = mock.Mock()
        self.api.dbapi.get_ports_by_node_id.return_value = []
        self.assertRaises(exception.NetworkError,
