...
class AgentDeploy(base.DeployInterface):
    ...
    def deploy(self, task):
        ...
        if task.node.power_state == states.POWER_OFF:
            ...
            manager_utils.node_power_action(task, task.node, states.POWER_ON)
    ...
    def tear_down(self, task):
        ...
        :returns: status of the deploy. One of ironic.common.states.
        """
        LOG.info(_('Tearing down node %s, powering off.'), task.node.id)
        # Reboot into ramdisk
    ...
    def prepare(self, task):
        ...
        if CONF.agent.dhcp_provider == 'neutron':
            LOG.info(_('Creating PXE config for node %s.'), task.node.id)
    ...
    def clean_up(self, task):
        ...
        if CONF.agent.dhcp_provider == 'neutron':
            LOG.info(_('Removing PXE config for node %s.'), task.node.id)
