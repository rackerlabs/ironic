...
agent_opts = [
    ...
    cfg.StrOpt('ironic_api_url',
               help='The address that will be sent to the agent which the '
                    ...
                    'to my_ip.',
               default='$my_ip'),
    cfg.IntOpt('agent_boot_max_wait',
               default=300,
               help='The amount of time in seconds to wait for an agent to '
                    'boot when using neutron as the dhcp_provider.')
    ]
...
class AgentDeploy(base.DeployInterface):
    ...
    def deploy(self, task, node):
        ...
        if node.power_state == states.POWER_OFF:
            ...
            manager_utils.node_power_action(task, node, states.POWER_ON)
    ...
    def tear_down(self, task, node):
        ...
        :returns: status of the deploy. One of ironic.common.states.
        """
        LOG.info(_('Tearing down node %s, powering off.'), node.id)
        # Reboot into ramdisk
        ...
        neutron_client.add_provisioning_network(node)
    ...
    def clean_up(self, task, node):
        ...
        if CONF.agent.dhcp_provider == 'neutron':
            LOG.info(_('Removing PXE config for node %s.'), node.id)
...
class AgentVendorInterface(base.VendorInterface):
    ...
    def validate(self, task, method, *args, **kwargs):
        """Validate the driver-specific Node deployment info.
    ...
    def _heartbeat(self, task, **kwargs):
        ...
        """
        node_object = task.node
        instance_info = node_object.instance_info
        driver_info = node_object.driver_info
        LOG.info(_(
            'Heartbeat from %(node)s, last heartbeat at %(heartbeat)s.'),
            {'node': node_object.uuid,
        ...
        node_object.instance_info = instance_info
        node_object.driver_info = driver_info
        node_object.save(task.context)

        if node_object.provision_state in (states.DELETING,):
            self._continue_tear_down(task, **kwargs)
        elif node_object.provision_state == states.DEPLOYWAIT:
            self._continue_deploy(task, **kwargs)

        elif (node_object.provision_state == states.DEPLOYING
                and self._deploy_is_done(node_object)):
            self._reboot_to_instance(task, **kwargs)

    def _deploy_is_done(self, node_object):
        return self._client.deploy_is_done(node_object)

    def _continue_deploy(self, task, **kwargs):
        node_object = task.node
        node_object.provision_state = states.DEPLOYING
        node_object.save(task.context)

        LOG.info(_('Continuing deploy for %s'), node_object.uuid)

        image_source = node_object.instance_info.get('image_source')
        configdrive = node_object.instance_info.get('configdrive')

        # Get the swift temp url
        glance = image_service.Service(version=2, context=task.context)
        image_info = glance.show(image_source)
        swift_temp_url = glance.swift_temp_url(image_info)
        LOG.debug(_('Got image info: %s'), image_info)
        image_info['urls'] = [swift_temp_url]

        # Tell the client to download and write the image with the given args
        res = self._client.prepare_image(node_object, image_info, configdrive)
        LOG.debug(_('prepare_image got response %s'), res)
        # TODO(jimrollenhagen) error checking

    def _check_deploy_success(self, node_object):
        # should only ever be called after we've validated that
        # the prepare_image command is complete
        command = self._client.get_commands_status(node_object)[-1]
    ...
    def _reboot_to_instance(self, task, **kwargs):
        ...
        LOG.info(_('Preparing to reboot to instance for node %s'),
                 node_object.uuid)
        error = self._check_deploy_success(node_object)
        if error is not None:
            # TODO(jimrollenhagen) power off if using neutron dhcp to
            #                      align with pxe driver?
            node_object.provision_state = states.DEPLOYFAIL
            node_object.target_provision_state = states.NOSTATE
            node_object.last_error = error
            node_object.save(task.context)
            return  # save error on node

        manager_utils.node_power_action(task, node_object, states.POWER_OFF)
        # Remove provisioning network, commit public network
        neutron_client = _get_neutron_client(task.context)
        neutron_client.remove_provisioning_network(node)
        neutron_client.add_public_network(node)

        LOG.info(_('Powering on node %s to finish deploying instance.'),
                 node_object.uuid)
        # TODO(jimrollenhagen) client.run_image(node_object)
        manager_utils.node_set_boot_device(task, 'hd')
        manager_utils.node_power_action(task, node_object, states.POWER_ON)

        node_object.provision_state = states.ACTIVE
        node_object.target_provision_state = states.NOSTATE
        node_object.save(task.context)
    ...
    def _continue_tear_down(self, task, **kwargs):
        ...
        node_object = task.node
        node_object.provision_state = states.DELETED
        node_object.target_provision_state = states.NOSTATE
        node_object.save(task.context)

        LOG.info(_('Continuing tear down for node %s'), node_object.uuid)
    ...
    def _lookup(self, context, **kwargs):
        ...
        LOG.info(_('Initial lookup for node %s succeeded.'), node_object.uuid)

        # TODO(JoshNang) save hardware
        # hardware = {
        #     'network': interfaces
        # }
        #
        # self._save_hardware(node, hardware, version)
        # node.save(context)
    ...
    def _find_node_by_macs(self, context, mac_addresses):
        ...
        try:
            node_object = node.Node.get_by_id(context, node_id)
