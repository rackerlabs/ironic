...
class AgentDeploy(base.DeployInterface):
    ...
    def tear_down(self, task, node):
        ...
        LOG.debug(_('Switching to provisioning network for node %s'),
                  node.uuid)
        # Remove public network, add provisioning network
        neutron_client = _get_neutron_client(task.context)
        neutron_client.deconfigure_instance_networks(node)
        neutron_client.add_provisioning_network(node)

        LOG.info(_('Powering on node %s to start decommissioning'), node.uuid)
        manager_utils.node_power_action(task, node, states.POWER_ON)
...
class AgentVendorInterface(base.VendorInterface):
    ...
    def _heartbeat(self, task, **kwargs):
        ...
        try:
            if node.provision_state in (states.DELETING,):
    ...
    def _continue_deploy(self, task, **kwargs):
        ...
        LOG.debug(_('prepare_image got response %(res)s for node %(node)s'),
                  {'res': res, 'node': node})
    ...
    def _reboot_to_instance(self, task, **kwargs):
        ...
        LOG.debug(_('Powering off node %s'), node.uuid)
        manager_utils.node_power_action(task, node, states.POWER_OFF)
        # Remove provisioning network, commit public network
        LOG.debug(_('Switching to public network for node %s'), node.uuid)
        neutron_client = _get_neutron_client(task.context)
        ...
        task.driver.ipmi_vendor._send_raw(task, '0x00 0x08 0x03 0x08')
        task.driver.ipmi_vendor._force_disk_boot(
        ...
        if version >= 2:
            for key in kwargs:
                if kwargs != 'interfaces':
                    hardware['hardware'][key] = kwargs[key]
