...
class AgentVendorInterface(base.VendorInterface):
    ...
    def _lookup_v0(self, task, **kwargs):
    ...
    def _find_node_by_macs(self, task, mac_addresses):
        ...
        nodes.
        """
        ports = self._find_ports_by_macs(task, mac_addresses)
        if not ports:
        ...
        try:
            node_object = node.Node.get_by_uuid(task.context, node_id)
    ...
    def _find_ports_by_macs(self, task, mac_addresses):
        """Given a list of MAC addresses, find the ports that match the MACs
        ...
        for mac in mac_addresses:
            ...
            try:
                ...
                port = task.dbapi.get_port(port_id=mac)
                ports.append(port)
