...
class AgentDeploy(base.DeployInterface):
    ...
    def tear_down(self, task, node):
        ...
        neutron_client.remove_public_network(node)
...
class AgentVendorInterface(base.VendorInterface):
    ...
    def _continue_tear_down(self, task, **kwargs):
        ...
        node = task.node
        node.provision_state = states.DELETED
        node.target_provision_state = states.NOSTATE
        node.save(task.context)

        LOG.info(_('Continuing tear down for node %s'), node.uuid)
