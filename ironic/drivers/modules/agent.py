...
agent_opts = [
    ...
    cfg.StrOpt('dhcp_provider',
               ...
               help='The service responsible for providing DHCP and TFTP to '
                    'booting agents. If set the "neutron", it expects a '
                    ...
                    'server which determines what TFTP image should be '
                    'booted.)'),
...
class AgentDeploy(base.DeployInterface):
    ...
    def deploy(self, task, node):
        ...
        if node.power_state == states.POWER_OFF:
            #TODO(JoshNang) Write out the TFTP file if driver is neutron
    ...
    def tear_down(self, task, node):
        ...
        # Switch to provisioning network
...
class AgentVendorInterface(base.VendorInterface):
    ...
    def _heartbeat(self, task, **kwargs):
        ...
        # TODO(JoshNang) should this be instance or driver info?
        instance_info['agent_last_heartbeat'] = int(_time())
        ...
        if node_object.provision_state in (states.DELETING, states.NOSTATE):
    ...
    def _continue_deploy(self, task, **kwargs):
        ...
        LOG.debug('Got image info: %s', image_info)
        ...
        LOG.debug('prepare_image got response %s', res)
    ...
    def _reboot_to_instance(self, task, **kwargs):
        ...
        manager_utils.node_power_action(task, node_object, states.OFF)
        ...
        manager_utils.node_power_action(task, node_object, states.ON)
