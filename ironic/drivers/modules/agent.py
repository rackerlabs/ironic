...
agent_opts = [
    ...
    cfg.StrOpt('dhcp_provider',
               ...
               help='The service responsible for providing DHCP and TFTP to'
                    ...
                    'server which determins what TFTP image should be '
                    'booted.)'),
    ...
    cfg.StrOpt('agent_kernel_args',
               help='Addtitional kernel args that will be passed to the '
                    'agent PXE config file. '),
...
class AgentDeploy(base.DeployInterface):
    ...
    def deploy(self, task, node):
        ...
        # machine, so we'll need to do some kind of smoke testing here.
    ...
    def tear_down(self, task, node):
        ...
        # TODO(JoshNang) switch to decom network in Neutron here
    ...
    def prepare(self, task, node):
        ...
        if CONF.agent.dhcp_provider == 'neutron':
            LOG.info('Booting agent via neutron')
            # Write out the TFTP file

            # Boot the box, wait for the first heartbeat, then deploy.
            manager_utils.node_set_boot_device(task, 'pxe')
            manager_utils.node_power_action(task, node, states.POWER_ON)

            # First heartbeat is required to continue with deploy.
            def _wait_for_heartbeat():
                if node.instance_info.get('agent_url'):
                    raise loopingcall.LoopingCallDone()
                if self.tries * 5 >= CONF.agent.agent_boot_max_wait:
                    msg = _('Timed out waiting for node %s to initially'
                            'boot agent.') % node.uuid
                    raise exception.IronicException(message=msg)
                self.tries += 1

            self.tries = 0
            timer = loopingcall.FixedIntervalLoopingCall(_wait_for_heartbeat)
            timer.start(interval=5).wait()
            while not node.instance_info.get('agent_url'):
                time.sleep(5)
...
class AgentVendorInterface(base.VendorInterface):
    ...
    def _lookup(self, context, **kwargs):
        ...
        # self._save_ports(context, node, interfaces)
        # node.save(context)
    ...
    def _save_ports(self, node, interfaces):
        #TODO(JoshNang) look for existing port first
        ports = []
        for interface in interfaces:
            port_dict = {
                'node_id': node.id,
                'address': interface.get('mac_address'),
                'extra': interface
            }
            ports.append(self.dbapi.create_port(port_dict))
        return ports

    def _flatten_dict(self, item, path='', flattened={}):
        """Yay recursion! Do a DFS through the 'tree' of the dictionary
        to flatten it out.
        ...
        if isinstance(item, str):
            ...
            if isinstance(item, dict):
                for k, v in item.items():
                    new_path = path + k
                    self._flatten_dict(v, new_path, flattened)
                return flattened
    ...
    def _save_hardware(self, context, node, hardware, version):
        ...
        in the database.
        """
        flattened_dict = self._flatten_dict(hardware, parent=None)
        for key, value in flattened_dict.items():
            key = '%(top_key)s/%(key)s' % {'top_key': 'hardware', 'key': key}
