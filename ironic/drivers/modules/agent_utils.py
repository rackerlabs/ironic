    ...
    def remove_provisioning_network(self, node):
        ...
        try:
            ...
            raise exception.NetworkError(_(
                ...
                'for %s from Neutron.') % node.uuid)

        if not ports or not ports.get('networks'):
            raise exception.NetworkError(_('No provisioning network ports '
                                           'attached to node %s') % node.uuid)
        if len(ports.get('networks')) > 1:
            LOG.warning(_('Multiple provisioning networks found for node %s, '
                          'attempting to remove all of them'),
                        node.uuid)
        for network in ports['networks']:
            try:
    ...
    def deconfigure_instance_networks(self, node):
        ...
        if not ports:
            raise exception.NetworkError(_(
                ...
                "node %s") % node.uuid)
        for port in self.dbapi.get_ports_by_node_id(node.id):
            vif_port_id = port['extra'].get('vif_port_id')
            ...
            try:
                self.client.update_port(vif_port_id, params)
            except neutron_exceptions.ConnectionFailed:
                raise exception.NetworkError(_(
