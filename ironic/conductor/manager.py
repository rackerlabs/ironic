...
class ConductorManager(periodic_task.PeriodicTasks):
    ...
    def do_node_deploy(self, context, node_id, rebuild=False):
        ...
        with task_manager.acquire(context, node_id, shared=False) as task:
            ...
            try:
                task.driver.deploy.validate_power_state(task, node, 'deploy')
            except exception.NodeInWrongPowerState as e:
                raise exception.InstanceDeployFailure(_(
                    "RPC do_node_deploy failed to validate power state."
                    "Error: %(msg)s") % {'msg': e}
                )
    ...
    def destroy_node(self, context, node_id):
        ...
        with task_manager.acquire(context, node_id) as task:
            ...
            task.driver.deploy.validate_power_state(task, node, 'destroy')
