...
class GlanceImageService(base_image_service.BaseImageService,
    ...
    def swift_temp_url(self, image_id, duration=None):
        ...
        if direct_url is None:
            raise exc.ImageUnacceptable('Images must have a direct_url to use '
                                        'swift temp urls.')
        ...
        for method in methods:
            if method not in valid_methods:
                raise exc.InvalidParameterValue(
                    _('swift_temp_url_methods must be in ') % valid_methods)
    ...
    def _swift_url_fragments(self, direct_url):
        ...
        try:
            ...
            raise exc.ImageUnacceptable('The direct URL could not be decoded')
        try:
            uuid.UUID(object_id)
        except ValueError:
